...
function filterImportantEmails(emails) {
  ...
  const promoKeywords = [
    'credit card', 'personal loan', 'pre-approved', 'apply for', 'ipo closing',
    'navigate market volatility', 'special offer', 'discount', 'voucher', 'reward',
    'newsletter', 'deal of the day', 'unsubscribe', 'cashback', 'exclusive deal',
    'limited time offer', 'sale live', 'buy now'
  ];

  // Common marketing / promotional sender patterns
  const promoSenderPatterns = [
    'info@', 'news@', 'mailer@', 'marketing@', 'digest@', 'newsletter@',
    'retailproducts@', 'promotions@', 'updates@', 'no-reply@kotak', 'offers@'
  ];
  ...
  const filtered = emails.filter((email) => {
    ...
    // Rule 1: Exclude if subject or body contains marketing keywords
    const matchesPromoKeyword = promoKeywords.some((kw) => subjectLower.includes(kw));
    if (matchesPromoKeyword) {
      return false;
    }

    // Rule 2: Exclude if sender matches marketing pattern
    const matchesPromoSender = promoSenderPatterns.some((pattern) => senderLower.includes(pattern));
    if (matchesPromoSender) {
      return false;
    }

    // Rule 3: Exclude if email contains bulk unsubscribe header
